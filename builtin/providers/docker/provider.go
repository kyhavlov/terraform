...
func providerConfigure(d *schema.ResourceData) (interface{}, error) {
	...
	for registry, auth := range authConfigs.Configs {
		registryClients[registry], err = rc.New(registry, auth.Username, auth.Password)

		if err != nil {
			return nil, fmt.Errorf("Error initializing registry client: %s", err)
		}
	}
